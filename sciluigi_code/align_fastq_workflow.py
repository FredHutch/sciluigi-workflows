#!/usr/bin/env python3
"""Align a set of samples against a single FASTA with BWA."""
...
from general_tasks import AlignFastqTask
from sra_tasks import ImportSRAFastq


class AlignFastsqWorkflow(sl.WorkflowTask):

    # If input files are supplied from SRA, fetch them
    # Align the WGS reads against a single FASTA with BWA
    ...
    def workflow(self):
        ...
        for sample_name in tasks_load_inputs:
            ...
            tasks_align_bwa[sample_name].in_fastq = tasks_load_inputs[sample_name].out_fastq

        return tasks_align_bwa


if __name__ == "__main__":
    """Map WGS data against a single FASTA file."""
    ...
    sl.run(
        main_task_cls=AlignFastsqWorkflow,
        cmdline_args=[
            "--{}={}".format(
                k.replace("_", "-"),
                v)
